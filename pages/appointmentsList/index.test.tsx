...
describe('Appointments', () => {
    ...
    it('updates the search input value when typing in the SearchBar', () => {
        const searchBar = screen.getByPlaceholderText('Search appointment...');
        fireEvent.change(searchBar, { target: { value: 'Search query' } });
        expect(searchBar).toHaveValue('Search query');
    });

    it('executes the search when the SearchBar value changes', () => {
        const searchBar = screen.getByPlaceholderText('Search appointment...');
        fireEvent.change(searchBar, { target: { value: 'Search query' } });
        // Add your assertions for the search functionality
    });

    it('renders the DoctorScheduler component', () => {
        expect(screen.getByTestId('doctor-scheduler')).toBeInTheDocument();
    });
});
